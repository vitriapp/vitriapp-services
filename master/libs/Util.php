...
class Util
{
    ...
    {
        $httpClientIp = filter_input(
            INPUT_SERVER,
            'HTTP_CLIENT_IP',
            FILTER_SANITIZE_STRING
        );

        $httpForwarder = filter_input(
            INPUT_SERVER,
            'HTTP_X_FORWARDED_FOR',
            FILTER_SANITIZE_STRING
        );

        $remoteAddress = filter_input(
            INPUT_SERVER,
            'REMOTE_ADDR',
            FILTER_SANITIZE_STRING
        );

        $ipClient = $httpClientIp;
        if (empty($ipClient) && empty($httpForwarder)) {
            $ipClient = $remoteAddress;
        }
